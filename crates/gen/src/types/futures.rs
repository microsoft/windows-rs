...
fn to_async_tokens(
    ...
    let wf = to_namespace_tokens("Windows.Foundation", &self_name.namespace);
    ...
    (
        quote! {
            pub fn get(&self) -> ::winrt::Result<#return_type> {
                if self.status()? == #wf AsyncStatus::Started {
                    unsafe {
                        let event = ::winrt::CreateEventW(::std::ptr::null_mut(), 1, 0, ::std::ptr::null_mut());
                        self.set_completed(#wf #handler::new(move |_sender, _args| {
