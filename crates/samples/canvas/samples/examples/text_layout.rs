...
fn draw(ctx: &DrawContext) {
    ...
    let Ok(format) =
        TextFormat::new("Segoe UI", 28.0).map(|f| f.with_word_wrapping(WordWrapping::Wrap))
    else {
        return;
    };
    let Ok(layout) = TextLayout::new(TEXT, &format, box_rect.width(), box_rect.height()) else {
        return;
    };
    let metrics = layout.metrics();

    let Ok(outline) = ctx.create_solid_brush(ColorF::from_rgb8(60, 70, 90)) else {
        return;
    };
    let Ok(accent) = ctx.create_solid_brush(ColorF::CORNFLOWER_BLUE) else {
        return;
    };
    let Ok(white) = ctx.create_solid_brush(ColorF::WHITE) else {
        return;
    };
    ...
    let Ok(label_format) = TextFormat::new("Consolas", 16.0) else {
        return;
    };
    let label = format!(
        "{} lines  -  {:.0} x {:.0} px",
