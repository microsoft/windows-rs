...
pub mod performance {
    ...
    impl Scenario {
        ...
        fn editor_sender(&self) -> LocalSender<Message> {
            self.props.sender.borrow().as_ref().unwrap().clone()
        }
    ...
    impl Component for LivePerformance {
        ...
        fn create(props: &Self::Props, _context: &mut ComponentContext<Self>) -> Self {
            let editor = EditorProps::default();
            let target_samples = props.samples;
            ...
            std::thread::spawn(move || {
                ...
                if !done.swap(true, Ordering::AcqRel) {
                    _ = std::fs::write(
                        ...
                        "windows-reactor-next live virtual editor\nno composition frames captured\n",
                    );
                    std::process::exit(2);
                }
            });
        ...
        fn view(&self, _props: &LiveProps, _context: &mut ViewContext<Self>) -> View {
    ...
    fn finish_live(stats: &mut LiveStats, done: &AtomicBool, result: Result<(), String>) {
        ...
        let report = match result {
            ...
            Err(error) => format!("windows-reactor-next live virtual editor\n{error}\n"),
        };
    ...
    fn live_report_path(active: bool) -> std::path::PathBuf {
        let name = if active {
            "reactor-next-virtual-live-perf.txt"
        } else {
            "reactor-next-virtual-live-baseline.txt"
        };
        std::env::current_exe()
            .unwrap()
            .parent()
    ...
    pub fn run_live(samples: usize, active: bool) {
        assert!(samples > 0);
        bootstrap().unwrap();
        App::run_component::<LivePerformance>(LiveProps { active, samples }).unwrap();
    }
}
