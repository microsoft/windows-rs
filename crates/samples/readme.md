...
| | `reactor-drag-drop` / `reactor-exit-transition` / `reactor-lightweight-resources` / `reactor-opacity-transition` / `reactor-responsive-navigation` / `reactor-scale-transition` / `reactor-theme-transition` | `cargo run -p <crate>` |
...
| [windows-notifyicon](../../docs/crates/windows-notifyicon.md) | `notifyicon-basic` / `reactor-notifyicon` | `cargo run -p <crate>` |
