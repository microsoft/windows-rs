...
//! sample: [`composition_host`] delivers a `CompositionHostHandle`, and the
//! optional [`windows-composition`](windows_composition) crate (built with its
//! `reactor` feature) supplies the safe, typed composition API via
//! [`CompositionHostExt`]. We obtain the host's [`Compositor`], build a
//! `SpriteVisual` tree, and attach it with `CompositionHostExt::set_child_visual`.
