...
use windows_reactor::{
    Application, FontWeight, StackPanel, TextBlock, Thickness, Window, button, component, hstack,
    run_reactor_winui_app,
};

fn counter(heading: String) -> windows_reactor::Element {
    component(move |cx| {
        ...
        let windows = ids
            ...
            .map(|id| {
                let content = if id == 0 {
                    ...
                    StackPanel::new([
                        TextBlock::new(
                            "Each window shares one Reactor and owns independent component state.",
                        )
                        .build(),
                        ...
                        button("Open counter window", move || {
                            let Some(next) = opened_for_open.try_value().map(|value| value + 1)
                            else {
                                return;
                            };
                ...
                Window::new(
                    if id == 0 {
                        "Reactor windows".to_string()
                    } else {
                        format!("Counter window #{id}")
                    },
                    content,
                    move || {
                        ids_for_close.update(|ids| {
