...
fn get_hardware_adapter(factory: &IDXGIFactory4) -> Result<IDXGIAdapter1> {
    unsafe {
        for i in 0.. {
            let adapter = Into::<&IDXGIFactory1>::into(factory).EnumAdapters1(i)?;

            let mut desc = std::mem::zeroed();
            adapter.GetDesc1(&mut desc)?;

            if DXGI_ADAPTER_FLAG(desc.Flags & DXGI_ADAPTER_FLAG_SOFTWARE.0)
...
mod d3d12_hello_triangle {
    ...
    impl DXSample for Sample {
        ...
        fn bind_to_window(&mut self, hwnd: &HWND) -> Result<()> {
            unsafe {
                ...
                let swap_chain: IDXGISwapChain3 = Into::<&IDXGIFactory2>::into(&self.dxgi_factory)
                    .CreateSwapChainForHwnd(&command_queue, *hwnd, &swap_chain_desc, None, None)?
                    .cast()?;

                // This sample does not support fullscreen transitions
                Into::<&IDXGIFactory>::into(&self.dxgi_factory)
                ...
                let render_targets: [ID3D12Resource; FRAME_COUNT as usize] =
                    array_init::try_array_init(|i: usize| -> Result<ID3D12Resource> {
                        let render_target: ID3D12Resource =
                            Into::<&IDXGISwapChain>::into(&swap_chain).GetBuffer(i as u32)?;
        ...
        fn render(&mut self) {
            if let Some(resources) = &mut self.resources {
                ...
                // Present the frame.
                unsafe { Into::<&IDXGISwapChain>::into(&resources.swap_chain).Present(1, 0) }
                    .ok()
                    .unwrap();
    ...
    fn create_pipeline_state(
        ...
        unsafe {
            ...
            let vertex_shader = D3DCompileFromFile(
                &shaders_hlsl,
                None,
                None,
                s!("VSMain"),
            ...
            let pixel_shader = D3DCompileFromFile(
                &shaders_hlsl,
                None,
                None,
                s!("PSMain"),
            ...
            let mut desc = D3D12_GRAPHICS_PIPELINE_STATE_DESC {
                ...
                DepthStencilState: std::mem::zeroed(),
