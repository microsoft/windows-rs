    ...
    for position in 0..params.len() {
        ...
        match params[position].kind {
            ...
            | SignatureParamKind::ArrayRelativeByteLen(relative) => {
                ...
                if !reader
                    .param_flags(params[relative].def)
            ...
            if signature_param_is_convertible(reader, param) {
                ...
                if param.ty.is_pointer()
                    && (flags.contains(ParamAttributes::Optional)
                        || reader.has_attribute(param.def, "ReservedAttribute"))
                {
                    param.kind = SignatureParamKind::OptionalPointer;
                } else if reader.type_is_primitive(&param.ty)
                    && (!param.ty.is_pointer() || reader.type_is_blittable(&param.ty.deref()))
                {
...
fn param_or_enum(reader: &Reader, row: Param) -> Option<String> {
    reader
        .find_attribute(row, "AssociatedEnumAttribute")
        .and_then(|attribute| {
...
pub fn signature_param_is_convertible(reader: &Reader, param: &SignatureParam) -> bool {
    !reader.param_flags(param.def).contains(ParamAttributes::Out)
        && !param.ty.is_winrt_array()
        && !param.ty.is_pointer()
        && !param.kind.is_array()
        && (type_is_borrowed(reader, &param.ty)
            || type_is_non_exclusive_winrt_interface(reader, &param.ty)
            || type_is_trivially_convertible(reader, &param.ty))
}
...
pub fn signature_kind(reader: &Reader, signature: &Signature) -> SignatureKind {
    ...
    match &signature.return_type {
        ...
        Type::HRESULT => {
            if signature.params.len() >= 2 {
                if let Some(guid) = signature_param_is_query_guid(reader, &signature.params) {
                    ...
                    {
                        if reader
                            .param_flags(signature.params[object].def)
        ...
        Type::TypeDef(def, _) if reader.type_def_type_name(*def) == TypeName::NTSTATUS => {
            SignatureKind::ResultVoid
        }
        Type::TypeDef(def, _) if reader.type_def_type_name(*def) == TypeName::WIN32_ERROR => {
            SignatureKind::ResultVoid
        }
        Type::TypeDef(def, _)
            if reader.type_def_type_name(*def) == TypeName::BOOL
                && method_def_last_error(reader, signature.def) =>
        {
            SignatureKind::ResultVoid
        }
...
fn signature_is_retval(reader: &Reader, signature: &Signature) -> bool {
    signature
        .params
        .last()
        .map_or(false, |param| signature_param_is_retval(reader, param))
...
fn signature_param_is_query_guid(reader: &Reader, params: &[SignatureParam]) -> Option<usize> {
    params.iter().rposition(|param| {
        param.ty == Type::ConstPtr(Box::new(Type::GUID), 1)
            && !reader.param_flags(param.def).contains(ParamAttributes::Out)
    })
}
fn signature_param_is_query_object(reader: &Reader, params: &[SignatureParam]) -> Option<usize> {
    params.iter().rposition(|param| {
        param.ty == Type::MutPtr(Box::new(Type::Void), 2)
            && reader.has_attribute(param.def, "ComOutPtrAttribute")
    })
}
...
fn type_def_is_callback(reader: &Reader, row: TypeDef) -> bool {
    !reader
        ...
        && reader.type_def_kind(row) == TypeKind::Delegate
}
...
pub fn type_def_has_callback(reader: &Reader, row: TypeDef) -> bool {
    ...
    fn check(reader: &Reader, row: TypeDef) -> bool {
        if reader
            .type_def_fields(row)
            .any(|field| type_has_callback(reader, &reader.field_type(field, Some(row))))
        {
            return true;
        }
        false
