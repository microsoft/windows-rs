...
pub fn writer(writer: &Writer, def: TypeDef) -> TokenStream {
    if writer.sys {
        if writer.reader.type_def_has_default_interface(def) {
...
fn gen_class(writer: &Writer, def: TypeDef) -> TokenStream {
    ...
    if writer.reader.type_def_has_default_interface(def) {
        let new = if writer.reader.type_def_has_default_constructor(def) {
            quote! {
...
fn gen_conversions(
    ...
    for interface in interfaces {
        if writer.reader.type_is_exclusive(&interface.ty) {
            continue;
        }
