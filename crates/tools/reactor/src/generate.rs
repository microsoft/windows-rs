...
mod tests {
    ...
    fn output_contains_all_control_rows() {
        ...
        assert_eq!(output.matches("ControlDescriptor").count(), 80);
        ...
        assert!(output.contains("pub struct ProgressRing"));
        assert!(output.contains("pub struct HyperlinkButton"));
        assert!(output.contains("pub struct AutoSuggestBox"));
        assert!(output.contains("pub struct PasswordBox"));
        assert!(output.contains("pub struct RadioButton"));
        assert!(output.contains("pub struct FlipView"));
        assert!(output.contains("pub struct Rectangle"));
        assert!(output.contains("pub struct Ellipse"));
        assert!(output.contains("pub struct Line"));
        assert!(output.contains("pub struct SymbolIcon"));
        assert!(
            compact.contains("pubfnpreferred_height(mutself,height:WindowTitleBarHeight)->Self")
        );
        assert!(compact.contains("capabilities:&[Capability::Layout,Capability::WindowTitleBar]"));
        assert!(output.contains("pub struct ImageIcon"));
        assert!(output.contains("pub struct RadioButtons"));
        assert!(output.contains("pub struct RepeatButton"));
        assert!(output.contains("pub struct InfoBadge"));
        assert!(output.contains("pub struct InfoBar"));
        assert!(output.contains("pub struct ContentDialog"));
        assert!(output.contains("pub enum ContentDialogResult"));
        assert!(output.contains("pub struct BreadcrumbBar"));
        assert!(output.contains("pub struct PersonPicture"));
        assert!(output.contains("pub struct ScrollView"));
        assert!(output.contains("pub struct ListBox"));
        assert!(output.contains("pub struct ListBoxItem"));
        assert!(output.contains("pub struct RatingControl"));
        assert!(output.contains("pub struct Expander"));
        assert!(output.contains("pub struct ComboBox"));
        assert!(output.contains("pub struct Pivot"));
        assert!(output.contains("pub struct PivotItem"));
        assert!(output.contains("pub struct TitleBar"));
        assert!(output.contains("struct TitleBarEvents"));
        assert!(output.contains("Rc :: make_mut"));
        assert!(!output.contains("struct ButtonEvents"));
        assert!(output.contains("pub enum ButtonStyle"));
        assert!(output.contains("pub fn style"));
        assert!(output.contains("pub struct NavigationView"));
        assert!(output.contains("pub struct NavigationViewItem"));
        assert!(output.contains("NavigationViewMenuItems"));
        assert!(output.contains("slot_is_collection"));
        assert!(output.contains("pub fn on_selected_tag_changed"));
        assert!(output.contains("pub struct Image"));
        assert!(output.contains("pub struct SelectorBar"));
        assert!(output.contains("pub struct SelectorBarItem"));
        assert!(output.contains("pub struct FontIcon"));
        assert!(output.contains("pub struct BitmapIcon"));
        assert!(output.contains("pub struct PathIcon"));
        assert!(
            compact.contains(
                "pubfnsource(mutself,value:implInto<String>,)->windows_core::Result<Self>"
            )
        );
        assert!(compact.contains(
            "pubfnsource_optional<T>(mutself,value:Option<T>,)->windows_core::Result<Self>"
        ));
        assert!(compact.contains(
            "pubfnsource_file(self,value:implAsRef<std::path::Path>,)->windows_core::Result<Self>"
        ));
        assert!(output.contains("pub fn on_is_pane_open_changed"));
        assert!(output.contains("NavigationViewIsPaneOpenChanged"));
        assert!(output.contains("pub struct ProgressBar"));
        assert!(output.contains("pub struct ToggleSwitch"));
        assert!(output.contains("pub struct CheckBox"));
        assert!(output.contains("pub struct ToggleButton"));
        assert!(output.contains("pub fn on_is_checked_changed"));
        ...
        assert!(!output.contains("ControlRole :: Controlled"));
        ...
        assert!(output.contains("Property < Brush >"));
        assert!(output.contains("pub fn background"));
        assert!(output.contains("pub fn border_brush"));
        assert!(output.contains("ThemeStyle :: new"));
        assert!(output.contains("PropertyValueRef :: Brush"));
