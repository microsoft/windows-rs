...
mod tests {
    ...
    fn resolves_pilot_schema() {
        ...
        assert_eq!(resolved.controls.len(), 78);
        ...
        let button_style = resolved.controls[1]
            .properties
            .iter()
            .find(|property| property.name == "Style")
            .unwrap();
        assert_eq!(button_style.value, "ButtonStyle");
        assert_eq!(button_style.adapter, Some(PropertyAdapter::ResourceStyle));
        assert_eq!(
            button_style.enum_variants,
            ["Default", "Accent", "Subtle", "TextLink"]
        );
        assert_eq!(
            button_style.resource_style_variants[1].resource.as_deref(),
            Some("AccentButtonStyle")
        );
        assert_eq!(resolved.controls[4].name, "Border");
        assert_eq!(resolved.controls[4].content.as_ref().unwrap().name, "Child");
        assert!(matches!(
            resolved.controls[4].content.as_ref().unwrap().target,
            SlotTarget::UiElement
        ));
        assert_eq!(resolved.controls[4].properties[0].value, "Thickness");
        assert_eq!(
            resolved.controls[4].properties[2].validation,
            Some(ValueValidation::FiniteNonNegative)
        );
        assert_eq!(resolved.controls[4].properties[3].value, "Brush");
        assert!(resolved.controls[4].properties[3].theme_style);
        assert_eq!(resolved.controls[4].properties[4].value, "Brush");
        assert!(resolved.controls[4].properties[4].theme_style);
        let drop_policy = resolved.controls[4]
            .properties
            .iter()
            .find(|property| property.field == "drop_policy")
            .unwrap();
        assert_eq!(drop_policy.name, "AllowDrop");
        assert_eq!(drop_policy.value, "DragDropPolicy");
        assert_eq!(drop_policy.adapter, Some(PropertyAdapter::DropPolicy));
        let tooltip = resolved
            .controls
            .iter()
            .find(|control| control.name == "ToolTip")
            .unwrap();
        assert_eq!(tooltip.name, "ToolTip");
        assert_eq!(tooltip.content.as_ref().unwrap().name, "Content");
        let dialog = resolved
            .controls
            .iter()
            .find(|control| control.name == "ContentDialog")
            .unwrap();
        assert_eq!(dialog.name, "ContentDialog");
        assert_eq!(dialog.lifecycle, Some(Lifecycle::ContentDialog));
        assert_eq!(dialog.events[0].payload, "ContentDialogResult");
        let text_box = resolved
            .controls
            .iter()
            .find(|control| control.name == "TextBox")
            .unwrap();
        assert_eq!(
            text_box.properties[0].feedback.as_deref(),
            Some("TextChanged")
        );
        assert_eq!(resolved.controls[7].name, "Grid");
        assert!(
            resolved.controls[7]
                .capabilities
                .contains(&Capability::GridDefinitions)
        );
        assert_eq!(text_box.events[0].payload, "Str");
        assert!(!text_box.properties[0].copy);
        assert!(matches!(
            text_box.events[0].source,
            EventPayloadSource::SenderProperty { .. }
        ));
        assert_eq!(resolved.controls[11].name, "NumberBox");
        assert_eq!(
        ...
        assert!(!resolved.controls[11].properties[0].observes_feedback);
        assert!(resolved.controls[11].properties[2].observes_feedback);
        assert_eq!(resolved.controls[12].name, "Slider");
        assert_eq!(
            resolved.controls[12].properties[0]
                .feedback_contract
                .unwrap(),
            FeedbackContract::SynchronousNormalized
        );
        assert!(!resolved.controls[12].properties[0].observes_feedback);
        assert!(resolved.controls[12].properties[2].observes_feedback);
        assert_eq!(resolved.controls[13].name, "TitleBar");
        assert!(matches!(resolved.controls[13].role, Role::Slots));
        assert!(
            resolved.controls[13]
                .capabilities
                .contains(&Capability::WindowTitleBar)
        );
        assert!(
            !resolved.controls[13]
                .capabilities
                .contains(&Capability::Reference)
        );
        assert_eq!(resolved.controls[13].slots.len(), 2);
        assert_eq!(resolved.controls[13].events.len(), 2);
        assert_eq!(resolved.controls[13].events[0].name, "BackRequested");
        assert_eq!(resolved.controls[13].events[1].name, "PaneToggleRequested");
        assert_eq!(resolved.controls[14].name, "NavigationView");
        assert!(matches!(resolved.controls[14].role, Role::Slots));
        assert_eq!(resolved.controls[14].slots.len(), 5);
        assert_eq!(resolved.controls[14].slots[0].name, "Content");
        assert!(
            resolved.controls[14].slots[0]
                .interface
                .ends_with("IContentControl")
        );
        assert_eq!(resolved.controls[14].slots[1].name, "Header");
        assert!(
            resolved.controls[14].slots[1]
                .interface
                .ends_with("INavigationView")
        );
        assert_eq!(resolved.controls[14].slots[2].name, "PaneCustomContent");
        assert!(
            resolved.controls[14].slots[2]
                .interface
                .contains("INavigationView")
        );
        assert_eq!(resolved.controls[14].slots[3].name, "PaneFooter");
        assert_eq!(resolved.controls[14].slots[4].name, "MenuItems");
        assert!(matches!(
            resolved.controls[14].slots[4].shape,
            SlotShape::Collection
        ));
        let navigation_selection = resolved.controls[14]
            .events
            .iter()
            .find(|event| event.name == "SelectionChanged")
            .unwrap();
        assert_eq!(
            navigation_selection.conversion,
            EventPayloadConversion::Selection
        );
        assert_eq!(navigation_selection.payload, "SelectionChange");
        let selection = resolved.controls[14].selection.as_ref().unwrap();
        assert_eq!(selection.slot, "MenuItems");
        assert_eq!(selection.item, "NavigationViewItem");
        assert!(selection.owner_interface.ends_with("INavigationView"));
        assert!(
            selection
                .selected_interface
                .ends_with("INavigationViewItemBase2")
        );
        assert!(selection.payload_interface.ends_with("IFrameworkElement"));
        let selector = resolved
            .controls
            .iter()
            .find(|control| control.name == "SelectorBar")
            .unwrap();
        assert_eq!(
            selector.slots[0].collection_item.as_deref(),
            Some("Microsoft.UI.Xaml.Controls.SelectorBarItem")
        );
        assert!(!selector.selection.as_ref().unwrap().payload_inspectable);
        let pane_open = resolved.controls[14]
            .properties
            .iter()
            .find(|property| property.name == "IsPaneOpen")
            .unwrap();
        assert_eq!(pane_open.feedback.as_deref(), Some("IsPaneOpenChanged"));
        assert_eq!(pane_open.clear_feedback, Some(true));
        let pane_changed = &resolved.controls[14].events[0];
        assert_eq!(pane_changed.payload, "Bool");
        assert!(matches!(
            pane_changed.source,
            EventPayloadSource::SenderProperty { .. }
        ));
        assert!(matches!(
            pane_changed.subscription,
            EventSubscription::PropertyChanged { .. }
        ));
        assert_eq!(resolved.controls[15].name, "NavigationViewItem");
        assert_eq!(
            resolved.controls[15].properties[0].adapter,
            Some(PropertyAdapter::InspectableString)
        );
        assert_eq!(resolved.controls[16].name, "SplitView");
        assert_eq!(
            resolved.controls[16].properties[2].value,
            "SplitViewDisplayMode"
        );
        assert_eq!(resolved.controls[16].slots.len(), 2);
        assert!(matches!(
            resolved.controls[16].slots[0].shape,
            SlotShape::Single(SlotTarget::UiElement)
        ));
        assert!(matches!(
            resolved.controls[16].slots[1].shape,
            SlotShape::Single(SlotTarget::UiElement)
        ));
        assert_eq!(resolved.controls[17].name, "ProgressBar");
        assert_eq!(resolved.controls[17].properties.len(), 7);
        assert_eq!(resolved.controls[17].properties[0].value, "F64");
        assert_eq!(resolved.controls[17].properties[3].value, "Bool");
        assert_eq!(resolved.controls[18].name, "ToggleSwitch");
        assert_eq!(resolved.controls[18].properties[0].value, "Bool");
        assert!(resolved.controls[18].properties[0].copy);
        assert_eq!(
            resolved.controls[18].properties[0].feedback.as_deref(),
            Some("Toggled")
        );
        assert_eq!(
            resolved.controls[18].properties[0]
                .feedback_contract
                .unwrap(),
            FeedbackContract::SynchronousExact
        );
        assert_eq!(resolved.controls[18].events[0].payload, "Bool");
        assert!(matches!(
            resolved.controls[18].events[0].source,
            EventPayloadSource::SenderProperty { .. }
        ));
        for control in [&resolved.controls[19], &resolved.controls[20]] {
            assert_eq!(control.properties[0].value, "Bool");
            assert!(control.properties[0].nullable_bool);
            assert_eq!(
                control.properties[0].feedback_contract,
                Some(FeedbackContract::SynchronousExact)
            );
            assert_eq!(
                control.events[0].conversion,
                EventPayloadConversion::Identity
            );
            assert!(matches!(
                control.events[0].subscription,
                EventSubscription::PropertyChanged { .. }
            ));
        }
        assert_eq!(resolved.controls[19].name, "CheckBox");
        assert_eq!(resolved.controls[20].name, "ToggleButton");
        assert!(matches!(resolved.controls[23].role, Role::Virtual));
        for name in ["TextBox", "NumberBox", "Slider"] {
            let control = resolved
                .controls
                .iter()
                .find(|control| control.name == name)
                .unwrap();
            assert!(matches!(control.role, Role::Slots));
        }
        assert!(matches!(resolved.controls[18].role, Role::Slots));
        assert_eq!(resolved.controls[18].slots.len(), 3);
        let image = resolved
            .controls
            .iter()
            .find(|control| control.name == "Image")
            .unwrap();
        assert_eq!(image.properties[0].value, "Str");
        assert_eq!(image.properties[0].adapter, Some(PropertyAdapter::ImageUri));
        assert_eq!(image.properties[0].validation, None);
        assert_eq!(image.properties[1].value, "Stretch");
        assert_eq!(resolved.controls[30].name, "ProgressRing");
        assert_eq!(resolved.controls[30].properties[2].name, "Value");
        assert_eq!(resolved.controls[30].properties[4].name, "IsActive");
        assert_eq!(resolved.controls[31].name, "ListBox");
        assert!(resolved.controls[31].slots[0].collection_cast);
        assert!(matches!(
            resolved.controls[31].events[0].source,
            EventPayloadSource::SenderProperty { .. }
        ));
        assert!(
            resolved
                .controls
                .iter()
                .any(|control| control.name == "ListBoxItem")
        );
    }
