windows_link::link!("srclient.dll" "system" fn SRRemoveRestorePoint(dwrpnum : u32) -> u32);
windows_link::link!("sfc.dll" "system" fn SRSetRestorePointA(prestoreptspec : *const RESTOREPOINTINFOA, psmgrstatus : *mut STATEMGRSTATUS) -> windows_sys::core::BOOL);
windows_link::link!("sfc.dll" "system" fn SRSetRestorePointW(prestoreptspec : *const RESTOREPOINTINFOW, psmgrstatus : *mut STATEMGRSTATUS) -> windows_sys::core::BOOL);
pub const ACCESSIBILITY_SETTING: u32 = 3;
pub const APPLICATION_INSTALL: RESTOREPOINTINFO_TYPE = 0;
pub const APPLICATION_RUN: u32 = 5;
pub const APPLICATION_UNINSTALL: RESTOREPOINTINFO_TYPE = 1;
pub const BACKUP: u32 = 15;
pub const BACKUP_RECOVERY: u32 = 14;
pub const BEGIN_NESTED_SYSTEM_CHANGE: RESTOREPOINTINFO_EVENT_TYPE = 102;
pub const BEGIN_NESTED_SYSTEM_CHANGE_NORP: u32 = 104;
pub const BEGIN_SYSTEM_CHANGE: RESTOREPOINTINFO_EVENT_TYPE = 100;
pub const CANCELLED_OPERATION: RESTOREPOINTINFO_TYPE = 13;
pub const CHECKPOINT: u32 = 7;
pub const CRITICAL_UPDATE: u32 = 18;
pub const DESKTOP_SETTING: u32 = 2;
pub const DEVICE_DRIVER_INSTALL: RESTOREPOINTINFO_TYPE = 10;
pub const END_NESTED_SYSTEM_CHANGE: RESTOREPOINTINFO_EVENT_TYPE = 103;
pub const END_SYSTEM_CHANGE: RESTOREPOINTINFO_EVENT_TYPE = 101;
pub const FIRSTRUN: u32 = 11;
pub const MANUAL_CHECKPOINT: u32 = 16;
pub const MAX_DESC: u32 = 64;
pub const MAX_DESC_W: u32 = 256;
pub const MAX_EVENT: u32 = 104;
pub const MAX_RPT: u32 = 18;
pub const MIN_EVENT: u32 = 100;
pub const MIN_RPT: u32 = 0;
pub const MODIFY_SETTINGS: RESTOREPOINTINFO_TYPE = 12;
pub const OE_SETTING: u32 = 4;
pub const RESTORE: u32 = 6;
#[repr(C, packed(1))]
#[derive(Clone, Copy)]
pub struct RESTOREPOINTINFOA {
    pub dwEventType: RESTOREPOINTINFO_EVENT_TYPE,
    pub dwRestorePtType: RESTOREPOINTINFO_TYPE,
    pub llSequenceNumber: i64,
    pub szDescription: [i8; 64],
}
impl Default for RESTOREPOINTINFOA {
    fn default() -> Self {
        unsafe { core::mem::zeroed() }
    }
}
#[repr(C, packed(1))]
#[derive(Clone, Copy)]
pub struct RESTOREPOINTINFOEX {
    pub ftCreation: super::super::Foundation::FILETIME,
    pub dwEventType: u32,
    pub dwRestorePtType: u32,
    pub dwRPNum: u32,
    pub szDescription: [u16; 256],
}
impl Default for RESTOREPOINTINFOEX {
    fn default() -> Self {
        unsafe { core::mem::zeroed() }
    }
}
#[repr(C, packed(1))]
#[derive(Clone, Copy)]
pub struct RESTOREPOINTINFOW {
    pub dwEventType: RESTOREPOINTINFO_EVENT_TYPE,
    pub dwRestorePtType: RESTOREPOINTINFO_TYPE,
    pub llSequenceNumber: i64,
    pub szDescription: [u16; 256],
}
impl Default for RESTOREPOINTINFOW {
    fn default() -> Self {
        unsafe { core::mem::zeroed() }
    }
}
pub type RESTOREPOINTINFO_EVENT_TYPE = u32;
pub type RESTOREPOINTINFO_TYPE = u32;
#[repr(C, packed(1))]
#[derive(Clone, Copy, Default)]
pub struct STATEMGRSTATUS {
    pub nStatus: super::super::Foundation::WIN32_ERROR,
    pub llSequenceNumber: i64,
}
pub const WINDOWS_BOOT: u32 = 9;
pub const WINDOWS_SHUTDOWN: u32 = 8;
pub const WINDOWS_UPDATE: u32 = 17;
