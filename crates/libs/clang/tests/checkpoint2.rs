use windows_clang::{Input, TypeRef, extract};
...
";
    ...
    let output = std::env::temp_dir().join(format!("windows-clang-{}.winmd", std::process::id()));
    windows_rdl::reader()
        .input_text(&rdl)
        .output(&output)
        .write()
    ...
    let scratch = std::env::temp_dir().join(format!("windows-clang-{}", std::process::id()));
