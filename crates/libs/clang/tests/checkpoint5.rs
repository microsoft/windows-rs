use windows_clang::{FactData, Input, extract};
...
#pragma pack(push, 4)
typedef struct alignas(16) _PACKED_4_ALIGNED {
    int first;
    long long second;
} PACKED_4_ALIGNED;
#pragma pack(pop)
...
struct IRedundantBase : IDerived, IBase {};
struct IUnknown {
...
struct IComMarker : IWriter, IUnknown {};
class IClassStyle : public IBase {
    virtual int ClassSlot() = 0;
};
...
";
    ...
    assert!(rdl.contains("#[packed(2)]\n    struct PACKED_2"));
    assert!(rdl.contains("#[packed(4)]\n    struct PACKED_4"));
    assert!(rdl.contains("#[align(16)]\n    struct ALIGNED"));
    assert!(rdl.contains("#[packed(1)]\n    #[align(8)]\n    struct PACKED_ALIGNED"));
    assert!(rdl.contains("#[packed(4)]\n    #[align(16)]\n    struct PACKED_4_ALIGNED"));
    ...
    assert!(
        rdl.contains(
            "extern \"C\" fn COMPARE_POINT(left: *const POINT, right: *const POINT) -> i32"
        )
    );
    assert!(rdl.contains(
    ...
    assert!(rdl.contains("#[no_guid]\n    interface IClassStyle: IBase"));
    assert!(rdl.contains("fn ClassSlot(&self) -> i32"));
    assert!(rdl.contains("#[no_guid]\n    interface IRedundantBase: IDerived"));
    assert!(rdl.contains("#[no_guid]\n    interface IComMarker: IWriter"));
    ...
    assert!(!rdl.contains("type PIBASE"));
    ...
    assert!(rdl.contains("fn AbiSlot(&self, value: i32) -> i32"));
    assert!(
...
#[test]
fn unrepresentable_sal_size_relations_are_reported() {
    helpers::ensure_libclang();
    ...
    assert_eq!(
        ...
        windows_clang::SalSizeValue::Expression("rows * columns".to_string())
    );
    assert!(
        expression
            .emit_with_library("Expression", "test.dll")
            .unwrap()
            .contains("#[in] buffer: *mut void")
    );
    ...
    assert_eq!(
        params[0].annotation.size.as_ref().unwrap().value,
        windows_clang::SalSizeValue::Expression("COUNT".to_string())
    );
    ...
    .unwrap();
    assert!(aligned.contains("#[align(8)]\n    struct BAD_LAYOUT"));
    ...
    let zero_output = std::env::temp_dir().join(format!(
        "windows-clang-zero-width-{}.winmd",
        std::process::id()
    ));
    windows_rdl::reader()
    ...
    let scratch =
        std::env::temp_dir().join(format!("windows-clang-record-alias-{}", std::process::id()));
...
struct IUnrelatedBases : IFirst, ISecond {};
";
    let snapshot = extract([Input::new("invalid.hpp", source)], &["-x", "c++"]).unwrap();
    for (name, expected) in [
        ...
        ("IUnrelatedBases", "interface has unrelated multiple bases"),
    ] {
