...
#[test]
fn public_data_only_cpp_classes_are_records() {
    helpers::ensure_libclang();
    ...
    let snapshot = extract(
        [Input::new(
            ...
            "typedef unsigned long DWORD;\n\
             ...
             public:\n\
                 DWORD id;\n\
                 union { void* pointer; DWORD value; };\n\
                 BOOL persistent;\n\
             };\n\
    ...
    .unwrap();
    let rdl = snapshot
        .emit_with_library("DataClasses", "api.dll")
        .unwrap();

    assert!(rdl.contains("struct CProperty"), "{rdl}");
    assert!(rdl.contains("id: u32"), "{rdl}");
    assert!(rdl.contains("pointer: *mut void"), "{rdl}");
    assert!(rdl.contains("value: u32"), "{rdl}");
    assert!(rdl.contains("persistent: BOOL"), "{rdl}");
    assert!(rdl.contains("type LPPROP = *mut CProperty"), "{rdl}");
    assert!(
        rdl.contains("fn Add(&self, value: *mut CProperty) -> i32"),
        "{rdl}"
    );
    assert!(
        rdl.contains("fn UseProperty(value: *mut CProperty, input: *const CProperty)"),
        "{rdl}"
    );
    assert!(rdl.contains("struct BitmapData"), "{rdl}");
    assert!(
        rdl.contains("fn UseBitmapData(value: *mut BitmapData)"),
        "{rdl}"
    );
}

#[test]
fn behavioral_cpp_classes_remain_opaque() {
    helpers::ensure_libclang();

    let rdl = extract(
        [Input::new(
            "behavioral_classes.hpp",
            "class PrivateData { int value; };\n\
             class ProtectedData { protected: int value; };\n\
             class MethodData { public: int value; void Reset(); };\n\
             class ConstructorData { public: ConstructorData(); int value; };\n\
             class DestructorData { public: ~DestructorData(); int value; };\n\
             class NonTrivialMember { public: NonTrivialMember(); int value; };\n\
             class NonPodWrapper { public: NonTrivialMember value; };\n\
    ...
    .unwrap()
    .emit_with_library("BehavioralClasses", "api.dll")
    .unwrap();
    ...
    for function in [
        ...
        assert!(
            rdl.contains(&format!("fn {function}(value: *mut void)")),
            "{rdl}"
        );
    }
}
...
#[test]
fn data_class_forward_declarations_use_the_definition() {
    helpers::ensure_libclang();

    let rdl = extract(
        [Input::new(
            "forward_data_class.hpp",
            "class Data;\n\
             typedef Data* PData;\n\
    ...
    .unwrap()
    .emit_with_library("ForwardDataClass", "api.dll")
    .unwrap();

    assert_eq!(rdl.matches("struct Data").count(), 1, "{rdl}");
    assert!(rdl.contains("value: u32"), "{rdl}");
    assert!(rdl.contains("type PData = *mut Data"), "{rdl}");
    assert!(rdl.contains("fn UseData(value: *mut Data)"), "{rdl}");
}
