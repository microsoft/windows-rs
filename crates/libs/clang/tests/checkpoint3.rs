...
fn planning_is_order_independent_and_dependencies_stay_tu_local() {
    ...
    let scratch = std::env::temp_dir().join(format!("windows-clang-plan-{}", std::process::id()));
...
";
    ...
    .emit("Incomplete")
    .unwrap();
    assert!(incomplete.contains("#[repr(u8)]"));
    assert!(incomplete.contains("enum Missing {\n"));
    windows_rdl::reader()
    ...
    assert!(
        unfixed
            .to_string()
            .contains("type root `Unfixed` is not emittable"),
        "{unfixed}"
    );
