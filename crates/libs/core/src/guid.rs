...
impl TryFrom<&str> for GUID {
    ...
    fn try_from(from: &str) -> Result<Self, HRESULT> {
        if from.len() != 36 {
...
fn try_u32(bytes: &mut core::str::Bytes<'_>, delimiter: bool) -> Result<u32, HRESULT> {
    next(bytes, 8, delimiter).ok_or_else(invalid_guid)
}

fn try_u16(bytes: &mut core::str::Bytes<'_>, delimiter: bool) -> Result<u16, HRESULT> {
    next(bytes, 4, delimiter)
        .map(|value| value as u16)
        .ok_or_else(invalid_guid)
}

fn try_u8(bytes: &mut core::str::Bytes<'_>, delimiter: bool) -> Result<u8, HRESULT> {
