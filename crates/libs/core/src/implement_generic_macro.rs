...
//!
//! Compared with [`implement_decl!`] the call-site has to spell out each interface as a
//! `base_ident: full_type` pair so the macro can use `base_ident` as the per-chain field
//! name without having to parse generic argument lists itself:
//!
//! ```rust,ignore
...
//!
//! The base ident of every interface entry must be unique within a single invocation —
...
macro_rules! implement_decl_generic {
    (
        ...
        // Capture the first interface so it can be used as the `Name` type argument to
        // `IInspectable_Vtbl::new`, matching the proc-macro's `GetRuntimeClassName`
        // behavior.
