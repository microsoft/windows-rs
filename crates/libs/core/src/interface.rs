...
pub unsafe trait Interface: Sized + Clone {
    ...
    ///
    /// Applications should generally _not_ call this method directly. Use
    ...
    /// the generic constraints. The returned `&dyn Any` refers to the _outer_ implementation
    /// object generated by `#[implement]` (e.g. `MyApp_Impl`), not the inner `MyApp` type.
    ///
    ...
    /// # Safety
    ///
    /// **IMPORTANT!** This uses a non-standard `QueryInterface` protocol identified by
    /// `DYNAMIC_CAST_IID` (there is no `IDynamicCast` interface). Objects that recognize
    /// `DYNAMIC_CAST_IID` store their `&dyn Any` directly at the interface pointer passed to
    /// `QueryInterface`. The returned value is twice the size of a normal pointer
    /// (`size_of::<&dyn Any>() == 2 * size_of::<*const c_void>()`), so callers cannot pass
    /// `&mut ptr` for an ordinary single-pointer-sized pointer. Only this method understands
    /// the protocol.
    ///
