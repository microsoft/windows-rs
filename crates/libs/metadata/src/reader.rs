...
impl Reader {
    ...
    pub fn remap_type(&self, type_name: &TypeName) -> Option<TypeName> {
        REMAP_TYPES
            .iter()
            .find_map(|(key, value)| (type_name == key).then(|| value.clone()))
    }
...
const REMAP_TYPES: [(TypeName, TypeName); 3] = [
    (TypeName::D2D_MATRIX_3X2_F, TypeName::Matrix3x2),
    (TypeName::D3DMATRIX, TypeName::Matrix4x4),
...
const SYS_CORE_TYPES: [(TypeName, Type); 10] = [
    (TypeName::GUID, Type::Name(TypeName::GUID)),
    (TypeName::IUnknown, Type::Name(TypeName::IUnknown)),
    (TypeName::HResult, Type::Name(TypeName::HResult)),
    (TypeName::HSTRING, Type::String),
