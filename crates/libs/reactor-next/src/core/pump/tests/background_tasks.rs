...
impl Component for BackgroundComponent {
    ...
    fn create(props: &Self::Props, context: &mut ComponentContext<Self>) -> Self {
        *props.sender.borrow_mut() = Some(context.sender());
        let handle = props.create.as_ref().map(|(barrier, value)| {
        ...
        *props.handle.borrow_mut() = handle.clone();
        Self {
            handle,
            props: props.clone(),
            text: "idle".to_string(),
    ...
    fn update(&mut self, message: Message, context: &mut ComponentContext<Self>) {
        match message {
            ...
            Message::Start(barrier, value) => {
                let handle = context.spawn_background(move |_cancellation| {
                    barrier.wait();
                    Message::Complete(value)
                });
                *self.props.handle.borrow_mut() = Some(handle.clone());
                self.handle = Some(handle);
            }
            Message::StartUntilCancelled(observed) => {
                let handle = context.spawn_background(move |cancellation| {
...
fn fixture() -> (
    ...
    pump.mount_view(View::component::<BackgroundComponent>(Props {
        create: None,
...
fn background_completion_routes_to_the_owning_component() {
    ...
    assert!(
        sender
            ...
            .as_ref()
            .unwrap()
            .send(Message::Start(Arc::clone(&barrier), "complete".to_string()))
    );
...
#[test]
fn rejected_dispatcher_wake_rejects_and_removes_completion() {
    let handle = Rc::new(RefCell::new(None));
    let sender = Rc::new(RefCell::new(None));
    ...
    assert!(
        sender
            ...
            .as_ref()
            .unwrap()
            .send(Message::Start(Arc::clone(&barrier), "ignored".to_string()))
    );
    assert_eq!(pump.dispatch_components(1), Ok(1));
