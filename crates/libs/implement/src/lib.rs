...
//! - `Foo_Impl` — the generated outer type that wraps `Foo` and adds vtable pointers, a
//!   reference count, and the identity vtable pointer. Users implement `IFoo_Impl`
//!   (generated by `#[interface]`) for `Foo_Impl`, not for `Foo`.
//!
//! ## Vtable layout
//!
...
//! 5. `count` — a `WeakRefCount`.
//!
//! When a COM caller holds an `IFoo*`, it points into the vtable-pointer field at offset
//! -k. To recover the `Foo_Impl` pointer, code in `gen_query_interface` and
//! `gen_impl_as_impl` subtracts `2 + k` pointer-sized units (the `+ 2` accounts for the
//! leading `base` and `identity` fields).
//!
