//! Shared orchestration for a libclang header scrape into committed RDL + winmd metadata.
//!
//! Both `tool_win32` and `tool_wdk` (and any third-party scraper) run the *same* pipeline:
//! provision the pinned libclang, parse one or more translation units per architecture,
//! route every declaration to its defining-header partition, arch-merge the per-arch results
//! so a symbol that exists on (or differs across) only a subset of arches gains
//! `SupportedArchitecture`, and re-derive a single unified winmd from the merged corpus.
//!
//! Everything that differs between scrapers is data, captured in [`Config`]: which headers to
//! bring into scope, the include/lib directories, the import libraries, the reachability
...
//! against it by bare name). The one thing this crate does *not* own is toolchain provisioning
//! — which NuGet packages and versions supply the headers and libs — because that is genuinely
//! per-scraper; each tool resolves those and hands the resolved paths to [`Config`].
...
/// happened in the caller); this crate only orchestrates clang and the RDL/winmd emission.
pub struct Config {
