...
#[cfg(feature = "test")]
pub fn read_property(handle: &Handle, property: PropertyId) -> Result<PropertyValue, RuntimeError> {
    match (handle, property) {
        (Handle::TextBox(control), PropertyId::TextBoxText) => control
            .cast::<ITextBox>()
            .and_then(|control| control.Text())
            .map(PropertyValue::Str)
            .map_err(native_error),
        (Handle::AutoSuggestBox(control), PropertyId::AutoSuggestBoxText) => control
            .cast::<IAutoSuggestBox>()
            .and_then(|control| control.Text())
            .map(PropertyValue::Str)
            .map_err(native_error),
        (Handle::PasswordBox(control), PropertyId::PasswordBoxPassword) => control
            .cast::<IPasswordBox>()
            .and_then(|control| control.Password())
            .map(PropertyValue::Str)
            .map_err(native_error),
        ...
        (Handle::Slider(control), PropertyId::SliderValue) => control
            ...
            .map_err(native_error),
        (Handle::NavigationView(control), PropertyId::NavigationViewIsPaneOpen) => control
            .cast::<INavigationView>()
            .and_then(|control| control.IsPaneOpen())
            .map(PropertyValue::Bool)
            .map_err(native_error),
        (Handle::SplitView(control), PropertyId::SplitViewIsPaneOpen) => control
            .cast::<ISplitView>()
            .and_then(|control| control.IsPaneOpen())
            .map(PropertyValue::Bool)
            .map_err(native_error),
        (Handle::ToggleSwitch(control), PropertyId::ToggleSwitchIsOn) => control
            .cast::<IToggleSwitch>()
            .and_then(|control| control.IsOn())
            .map(PropertyValue::Bool)
            .map_err(native_error),
        (Handle::CheckBox(control), PropertyId::CheckBoxIsChecked) => control
            .cast::<IToggleButton>()
            .and_then(|control| control.IsChecked())
            .map(PropertyValue::Bool)
            .map_err(native_error),
        (Handle::ToggleButton(control), PropertyId::ToggleButtonIsChecked) => control
            .cast::<IToggleButton>()
            .and_then(|control| control.IsChecked())
            .map(PropertyValue::Bool)
            .map_err(native_error),
        (Handle::RadioButton(control), PropertyId::RadioButtonIsChecked) => control
            .cast::<IToggleButton>()
            .and_then(|control| control.IsChecked())
            .map(PropertyValue::Bool)
            .map_err(native_error),
        ...
        (Handle::ColorPicker(control), PropertyId::ColorPickerColor) => control
            ...
            .map(|value| {
                PropertyValue::Color(crate::Color {
                    a: value.a,
                    r: value.r,
                    g: value.g,
                    b: value.b,
