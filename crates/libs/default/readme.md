...
Most callers should use `.input_default()` on `windows-bindgen`, `windows-rdl`, `windows-clang`, or
`windows-csharp`. Use this crate directly when implementing another tool that accepts metadata
bytes.
