...
provide uninitialized storage. Only unsigned count parameters become slice lengths; signed counts
remain explicit because they may use negative sentinel values. A trailing retval must be
output-only, required, non-reserved, uncounted, and pointer-shaped. The existing void-pointee and
size limits apply only to unmarked heuristic candidates.
