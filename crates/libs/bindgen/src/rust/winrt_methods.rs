        ...
        let param = if param.def.flags().contains(ParamAttributes::In) {
            if param.ty.is_winrt_array() {
                if type_is_blittable(&param.ty) {
                    quote! { #name.len().try_into()?, #name.as_ptr(), }
                } else {
                    quote! { #name.len().try_into()?, ::core::mem::transmute(#name.as_ptr()), }
                }
            ...
            if type_is_blittable(&param.ty) {
                quote! { #name.len().try_into()?, #name.as_mut_ptr(), }
            } else {
                quote! { #name.len().try_into()?, ::core::mem::transmute_copy(&#name), }
            }
        } else if param.ty.is_winrt_array_ref() {
