...
impl CppStruct {
    ...
    pub fn is_handle(&self) -> bool {
        ...
        if field.name() != "Value" || !field.field_ty(Some(self)).is_primitive() {
            return false;
        }
    ...
    fn write_with_cfg(&self, config: &Config, cfg: &TokenStream) -> TokenStream {
        ...
        let fields: Vec<_> = self
            ...
            .map(|field| (field.name(), field.field_ty(Some(self))))
            .collect();
    ...
    fn can_derive_default(&self, config: &Config) -> bool {
        !self.has_explicit_layout()
            && !self.def.fields().any(|field| {
                let ty = field.field_ty(Some(self));
    ...
    pub fn is_copyable(&self) -> bool {
        ...
        self.def
            .fields()
            .all(|field| field.field_ty(Some(self)).is_copyable())
    }
    ...
    fn multi_struct_fields(&self) -> impl Iterator<Item = Self> + '_ {
        self.def
            .fields()
            .map(|field| field.field_ty(Some(self)))
    ...
    pub fn size(&self) -> usize {
        if self.def.flags().contains(TypeAttributes::ExplicitLayout) {
            self.def
                .fields()
                .map(|field| field.field_ty(Some(self)).size())
                .max()
                .unwrap_or(1)
        } else {
            let mut sum = 0;
    ...
    pub fn align(&self) -> usize {
        self.def
            .fields()
            .map(|field| field.field_ty(Some(self)).align())
            .max()
            .unwrap_or(1)
    }
}
...
impl Dependencies for CppStruct {
    fn combine(&self, dependencies: &mut TypeMap) {
        for field in self.def.fields() {
            field.field_ty(Some(self)).combine(dependencies);
        }
