    ...
    fn write_with_cfg(&self, config: &Config, cfg: &TokenStream) -> TokenStream {
        ...
        let fields = {
            let fields = fields.iter().map(|(name, ty)| {
                ...
                let ty = if !config.mode.is_sys() && is_union && !ty.is_copyable(config.reader) {
                    let ty = ty.write_default(config);
                    quote! { core::mem::ManuallyDrop<#ty> }
                } else if !config.mode.is_sys() && ty.is_dropped(config.reader) {
        ...
        if !config.mode.is_sys() && !has_explicit_layout && !has_packing {
            derive.extend(["Debug"]);
