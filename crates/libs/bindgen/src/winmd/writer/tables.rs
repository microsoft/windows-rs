...
impl Tables {
    pub fn into_stream(self) -> Vec<u8> {
        ...
        let resolution_scope = metadata::coded_index_size(&[
            self.Module.len(),
            self.ModuleRef.len(),
            self.AssemblyRef.len(),
            self.TypeRef.len(),
        ]);
        let type_def_or_ref = metadata::coded_index_size(&[
            self.TypeDef.len(),
            self.TypeRef.len(),
            self.TypeSpec.len(),
        ]);
        let has_constant =
            metadata::coded_index_size(&[self.Field.len(), self.Param.len(), self.Property.len()]);
        let type_or_method_def =
            metadata::coded_index_size(&[self.TypeDef.len(), self.MethodDef.len()]);
        let member_ref_parent = metadata::coded_index_size(&[
            self.TypeDef.len(),
            self.TypeRef.len(),
            self.ModuleRef.len(),
            self.MethodDef.len(),
        ...
        let custom_attribute_type =
            metadata::coded_index_size(&[self.MethodDef.len(), self.MemberRef.len(), 0, 0, 0]);
