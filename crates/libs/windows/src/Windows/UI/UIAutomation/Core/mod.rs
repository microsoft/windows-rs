...
impl ICoreAutomationRemoteOperationExtensionProvider {
    ...
    {
        let this = self;
        unsafe { (::windows_core::Interface::vtable(this).CallExtension)(::windows_core::Interface::as_raw(this), extensionid, context.into_param().abi(), operandids.len().try_into()?, operandids.as_ptr()).ok() }
    }
...
impl CoreAutomationRemoteOperation {
    ...
    pub fn Execute(&self, bytecodebuffer: &[u8]) -> ::windows_core::Result<AutomationRemoteOperationResult> {
        ...
        unsafe {
            let mut result__ = ::std::mem::zeroed();
            (::windows_core::Interface::vtable(this).Execute)(::windows_core::Interface::as_raw(this), bytecodebuffer.len().try_into()?, bytecodebuffer.as_ptr(), &mut result__).from_abi(result__)
        }
    }
