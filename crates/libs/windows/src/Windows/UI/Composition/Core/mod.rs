...
impl CompositorController {
    ...
    pub fn Commit(&self) -> Result<(), windows_result::HRESULT> {
        let this = self;
        unsafe { (windows_core::Interface::vtable(this).Commit)(windows_core::Interface::as_raw(this)).ok() }
    }
    pub fn EnsurePreviousCommitCompletedAsync(&self) -> Result<windows_future::IAsyncAction, windows_result::HRESULT> {
        let this = self;
        unsafe {
            let mut result__ = core::mem::zeroed();
    ...
    pub fn CommitNeeded<P0>(&self, handler: P0) -> Result<i64, windows_result::HRESULT>
    where
    ...
    pub fn RemoveCommitNeeded(&self, token: i64) -> Result<(), windows_result::HRESULT> {
        let this = self;
