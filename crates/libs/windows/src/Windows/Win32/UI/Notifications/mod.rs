...
impl INotificationActivationCallback {
    pub unsafe fn Activate(&self, appusermodelid: windows_core::PCWSTR, invokedargs: windows_core::PCWSTR, data: &[NOTIFICATION_USER_INPUT_DATA]) -> windows_core::Result<()> {
        unsafe { (windows_core::Interface::vtable(self).Activate)(windows_core::Interface::as_raw(self), core::mem::transmute(appusermodelid), core::mem::transmute(invokedargs), core::mem::transmute(data.as_ptr()), data.len().try_into().unwrap()).ok() }
    }
}
#[repr(C)]
#[doc(hidden)]
