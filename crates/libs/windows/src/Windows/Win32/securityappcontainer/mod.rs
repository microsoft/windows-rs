...
pub unsafe fn GetAppContainerNamedObjectPath(token: Option<super::winnt::HANDLE>, appcontainersid: Option<super::winnt::PSID>, objectpath: Option<&mut [u16]>, returnlength: *mut u32) -> windows_core::BOOL {
    ...
    unsafe { GetAppContainerNamedObjectPath(token.unwrap_or(core::mem::zeroed()) as _, appcontainersid.unwrap_or(core::mem::zeroed()) as _, objectpath.as_deref().map_or(0, |slice| slice.len().try_into().unwrap()), core::mem::transmute(objectpath.as_deref().map_or(core::ptr::null(), |slice| slice.as_ptr())), returnlength as _) }
}
