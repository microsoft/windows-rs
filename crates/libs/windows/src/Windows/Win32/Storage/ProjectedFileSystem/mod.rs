...
#[inline]
pub unsafe fn PrjClearNegativePathCache(namespacevirtualizationcontext: PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT) -> windows_core::Result<u32> {
    ...
    unsafe {
        ...
        PrjClearNegativePathCache(namespacevirtualizationcontext, &mut result__).map(|| result__)
    }
}
#[inline]
pub unsafe fn PrjCompleteCommand(namespacevirtualizationcontext: PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT, commandid: i32, completionresult: windows_core::HRESULT, extendedparameters: *const PRJ_COMPLETE_COMMAND_EXTENDED_PARAMETERS) -> windows_core::Result<()> {
    ...
    unsafe { PrjCompleteCommand(namespacevirtualizationcontext, commandid, completionresult, extendedparameters).ok() }
}
#[inline]
pub unsafe fn PrjDeleteFile<P1>(namespacevirtualizationcontext: PRJ_NAMESPACE_VIRTUALIZATION_CONTEXT, destinationfilename: P1, updateflags: PRJ_UPDATE_TYPES) -> windows_core::Result<PRJ_UPDATE_FAILURE_CAUSES>
where
    P1: windows_core::Param<windows_core::PCWSTR>,
{
    ...
    unsafe {
        let mut result__ = core::mem::zeroed();
        PrjDeleteFile(namespacevirtualizationcontext, destinationfilename.param().abi(), updateflags, &mut result__).map(|| result__)
    }
}
#[inline]
...
#[inline]
pub unsafe fn PrjFillDirEntryBuffer<P0>(filename: P0, filebasicinfo: *const PRJ_FILE_BASIC_INFO, direntrybufferhandle: PRJ_DIR_ENTRY_BUFFER_HANDLE) -> windows_core::Result<()>
where
    P0: windows_core::Param<windows_core::PCWSTR>,
{
    ...
    P0: windows_core::Param<windows_core::PCWSTR>,
{
    ...
    unsafe {
        let mut result__ = core::mem::zeroed();
        PrjStartVirtualizing(virtualizationrootpath.param().abi(), callbacks, instancecontext, options, &mut result__).map(|| result__)
    }
}
#[inline]
