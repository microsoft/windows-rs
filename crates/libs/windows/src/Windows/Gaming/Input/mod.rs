...
impl RawGameController {
    ...
    pub fn GetCurrentReading(&self, buttonarray: &mut [bool], switcharray: &mut [GameControllerSwitchPosition], axisarray: &mut [f64]) -> ::windows_core::Result<u64> {
        ...
        unsafe {
            let mut result__ = ::std::mem::zeroed();
            (::windows_core::Interface::vtable(this).GetCurrentReading)(::windows_core::Interface::as_raw(this), buttonarray.len().try_into()?, buttonarray.as_mut_ptr(), switcharray.len().try_into()?, switcharray.as_mut_ptr(), axisarray.len().try_into()?, axisarray.as_mut_ptr(), &mut result__).from_abi(result__)
        }
    }
