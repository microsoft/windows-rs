...
pub unsafe fn GetStringTypeExW(locale: super::winnt::LCID, dwinfotype: u32, lpsrcstr: *const u16, lpchartype: &mut [u16]) -> windows_core::BOOL {
    ...
    unsafe { GetStringTypeExW(locale, dwinfotype, lpsrcstr, lpchartype.len().try_into().unwrap(), core::mem::transmute(lpchartype.as_ptr())) }
}
#[inline]
...
pub unsafe fn MultiByteToWideChar(codepage: u32, dwflags: u32, lpmultibytestr: *const i8, cbmultibyte: i32, lpwidecharstr: Option<&mut [u16]>) -> i32 {
    ...
    unsafe { MultiByteToWideChar(codepage, dwflags, lpmultibytestr, cbmultibyte, core::mem::transmute(lpwidecharstr.as_deref().map_or(core::ptr::null(), |slice| slice.as_ptr())), lpwidecharstr.as_deref().map_or(0, |slice| slice.len().try_into().unwrap())) }
}
#[inline]
