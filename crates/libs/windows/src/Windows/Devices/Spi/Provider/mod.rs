...
impl ISpiProvider {
    pub fn GetControllersAsync(&self) -> windows_core::Result<windows_async::IAsyncOperation<windows_collections::IVectorView<ISpiControllerProvider>>> {
        let this = self;
        unsafe {
            let mut result__ = core::mem::zeroed();
...
pub trait ISpiProvider_Impl: windows_core::IUnknownImpl {
    fn GetControllersAsync(&self) -> windows_core::Result<windows_async::IAsyncOperation<windows_collections::IVectorView<ISpiControllerProvider>>>;
}
