...
pub(crate) unsafe fn take_result(value: Result<PWSTR>) -> String {
    value
...
pub(crate) unsafe fn take(value: PWSTR) -> String {
    if value.is_null() {
        return String::new();
    }

    let result = unsafe { PCWSTR::from_raw(value.0).to_string().unwrap_or_default() };
    unsafe { CoTaskMemFree(value.0 as *mut _) };
    result
}
...
pub(crate) unsafe fn allocate(value: &str) -> Result<PWSTR> {
