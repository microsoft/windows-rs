...
> **Status:** experimental and pre-release (`0.0.0`). The API is evolving and the crate is not yet published to crates.io. It currently covers environment creation (with options: user data folder, browser arguments, language) and controller creation, navigation and history (navigate, reload, stop, back/forward), document state (source URI, title), settings (script, dev tools, context menus, …), script execution and document-created scripts, events (navigation starting/completed, content loading, document title changed, web message received, window close requested, and deferral-based new window and permission requests), and host ↔ JavaScript messaging; more of the WebView2 surface will follow.
