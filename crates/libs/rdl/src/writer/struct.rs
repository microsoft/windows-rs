...
fn write_field(
    ...
    let ty = match item.ty() {
        ...
        metadata::Type::ArrayFixed(ty, len) => {
            if let metadata::Type::Name(ty_name) = ty.as_ref() {
                let len = Literal::usize_unsuffixed(len);
                if ty_name.namespace.is_empty() && !ty_name.name.contains('/') {
                    let nested = item
                        .index()
                        .nested(*parent)
                        .find(|t| t.name() == ty_name.name)
                        .unwrap_or_else(|| {
                            panic!("Could not resolve nested type: {}", ty_name.name)
                        });
                    ...
                    for segment in segments {
                        resolved = item
                            ...
                            .find(|t| t.name() == segment)
                            .unwrap_or_else(|| {
                                panic!("Could not resolve nested type: {}", segment)
                            });
                    }
