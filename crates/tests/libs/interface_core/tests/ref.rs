...
unsafe trait ITest: IUnknown {
    ...
    unsafe fn result_hstring(&self, input: Ref<HSTRING>, output: OutRef<HSTRING>) -> Result<(), HRESULT>;
    unsafe fn result_interface(&self, input: Ref<ITest>, output: OutRef<ITest>) -> Result<(), HRESULT>;
    unsafe fn result_required_input(&self, input: Ref<ITest>, output: OutRef<ITest>) -> Result<(), HRESULT>;
}
