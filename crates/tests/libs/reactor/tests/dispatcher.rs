...
fn run_on_demand_dispatcher_drains_normal_in_fifo_order() {
    ...
    dispatcher.enqueue(DispatchPriority::Normal, job(Rc::clone(&log), "a"));
    dispatcher.enqueue(DispatchPriority::Normal, job(Rc::clone(&log), "b"));
    dispatcher.enqueue(DispatchPriority::Normal, job(Rc::clone(&log), "c"));
...
fn run_on_demand_dispatcher_drains_normal_before_low() {
    ...
    dispatcher.enqueue(DispatchPriority::Normal, job(Rc::clone(&log), "n1"));
    dispatcher.enqueue(DispatchPriority::Low, job(Rc::clone(&log), "lo"));
    dispatcher.enqueue(DispatchPriority::Normal, job(Rc::clone(&log), "n2"));
    let fired = dispatcher.drain();
    assert_eq!(fired, 3);
