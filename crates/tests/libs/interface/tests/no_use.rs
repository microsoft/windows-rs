...
unsafe trait ITestPersistMemory: windows::objidl::IPersist {
    unsafe fn IsDirty(&self) -> windows::core::HRESULT;
}

#[windows::core::implement(ITestPersistMemory, windows::objidl::IPersist)]
struct Test;

impl windows::objidl::IPersist_Impl for Test_Impl {
...
fn test() -> windows::core::Result<()> {
    unsafe {
        let p: windows::objidl::IPersist = Test.into();
        assert_eq!(
            p.GetClassID()?,
            "CEE1D356-0860-4262-90D4-C77423F0E352".try_into()?
        );
        ...
        assert_eq!(m.IsDirty(), windows::winerror::S_FALSE);
