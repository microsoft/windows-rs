pub mod Test {
    ...
    pub trait ILeaf_Impl: IMiddle_Impl + IBase_Impl {
    ...
    pub trait IMiddle_Impl: IBase_Impl {
        fn MiddleMethod(&self) -> windows_result::Result<i32>;
    }
    impl IMiddle_Vtbl {
        pub const fn new<Identity: IMiddle_Impl, const OFFSET: isize>() -> Self {
            unsafe extern "system" fn MiddleMethod<Identity: IMiddle_Impl, const OFFSET: isize>(
                this: *mut core::ffi::c_void,
                result__: *mut i32,
            ) -> windows_core::HRESULT {
                unsafe {
                    let this: &Identity =
                        &*((this as *const *const ()).offset(OFFSET) as *const Identity);
                    match IMiddle_Impl::MiddleMethod(this) {
                        Ok(ok__) => {
                            result__.write(ok__);
                            windows_core::HRESULT(0)
                        }
            ...
            Self {
                ...
                MiddleMethod: MiddleMethod::<Identity, OFFSET>,
            }
        }
        pub fn matches(iid: &windows_core::GUID) -> bool {
