...
impl ITest {
    pub fn Numerics(&self, n: Vector2) -> windows_result::Result<()> {
        let this = self;
        unsafe {
    ...
    pub fn Async(&self) -> windows_result::Result<IAsyncAction> {
        let this = self;
        unsafe {
            let mut result__ = core::mem::zeroed();
...
pub trait ITest_Impl: windows_core::IUnknownImpl {
    fn Numerics(&self, n: &Vector2) -> windows_result::Result<()>;
    fn Collections(&self, c: windows_core::Ref<IVector<i32>>) -> windows_result::Result<()>;
    fn Async(&self) -> windows_result::Result<IAsyncAction>;
