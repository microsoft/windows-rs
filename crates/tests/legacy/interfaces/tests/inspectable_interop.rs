...
fn test() {
    ...
    assert_eq!(std::mem::size_of::<ISystemMediaTransportControlsInterop_abi>(), 7 * std::mem::size_of::<usize>());

    // And ICompositorInterop derives directly from IUnknown.
    // 3 for IUnknown and 3 for ICompositorInterop.
    assert_eq!(std::mem::size_of::<ICompositorInterop_abi>(), 6 * std::mem::size_of::<usize>());
}
