...
fn writer() {
    let temp_file = std::env::temp_dir().join("test_metadata.winmd");
    {
        ...
        let mut closable = TypeDef::new(TypeName::new("TestWindows.Foundation", "IClosable"));
    ...
    }
    {
        ...
        let def = reader.get(TypeName::new("TestWindows.Foundation", "IClosable")).next().unwrap();
        assert_eq!(reader.type_def_kind(def), TypeKind::Interface);
        assert!(reader.type_def_flags(def).winrt());
