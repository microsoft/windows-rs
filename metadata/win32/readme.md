...
headers. The whole API lives in a **single flat `Windows.Win32` namespace**, split
...
source (the clang cursor's defining header), not a hand-curated namespace layout.
...
scraper design - how the metadata matches the SDK headers, header partitioning, and the
documented deviations from the reference - and [`docs/crates/windows-rdl.md`](../../docs/crates/windows-rdl.md)
