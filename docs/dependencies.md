...
`reactor-setup` is a published runtime helper with no generated artifact, so its pins can't be
proven by regen. Instead **`tool_reactor` guards them** on every run, asserting (and failing loudly
on drift) that:
