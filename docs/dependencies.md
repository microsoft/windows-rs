...
tooling depend on: the version, where it is pinned, how it is obtained, and how the pin is validated. It covers *build/tooling and runtime* dependencies (Windows SDK, WDK, WinRT
...
silently change the generated metadata.
...
- **Pinned libclang:** like `tool_win32`/`tool_wdk`, `tool_webview` calls `ensure_libclang` +
  `assert_libclang_version`, so it parses with the exact pinned `22.1.8` (its `gen.yml` job needs
...
into the committed metadata at `crates/tools/reactor/winmd/`. It also refreshes the committed
...
not a package.) The metadata stays committed because `tool_webview` and `tool_composition` also
read it.
...
- **`reactor-setup`'s staging** - runs in the `build.rs` of every consuming app to stage the App
  SDK runtime and WebView2 projection. Dependency-free (std + the `curl.exe`/`tar.exe`
