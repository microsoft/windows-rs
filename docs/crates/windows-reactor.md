...
`crates/tools/reactor/src/winui.toml` plus the WinUI `.winmd` metadata. That
metadata is **not vendored**: `tool_reactor` downloads it from NuGet on demand
(pinned by the `WINAPPSDK_VER` / `WEBVIEW2_VER` constants in
`crates/tools/reactor/src/stage.rs`, staged into `target/reactor/winmd/`), the same
way `tool_win32`/`tool_windows` fetch the Windows SDK metadata — see
[dependencies.md](../dependencies.md). The generated files are:
...
### Runtime staging assets

[`windows-reactor-setup`](windows-reactor-setup.md) stages the Windows App SDK
runtime next to the built executable. Its per-architecture
`Microsoft.WindowsAppRuntime.Bootstrap.dll` is refreshed from the pinned
`RUNTIME_VER` package on each version bump, but two of its committed assets —
`assets/app.manifest` and `assets/resources.pri` — are **generated activation
assets with no committed generator**. `app.manifest` is a hand/tool transform of
the Windows App SDK `runtimes-framework/package.appxfragment` files into the SxS
fusion-manifest format (source versions are recorded in its header comment). It
maps WinRT class names to runtime DLLs and is forward-compatible, so it is
refreshed only when the curated control set needs classes a newer Windows App SDK
moved or added — not on every bump.

**Future work:** build a committed generator (a `tool_*`) that regenerates
`app.manifest` from the packaged appxfragments and produces `resources.pri`
reproducibly, so a Windows App SDK version bump refreshes these assets
mechanically (like `tool_reactor`/`tool_bindings` do for the metadata) instead of
relying on a manual, undocumented process. See
[dependencies](../dependencies.md) for the full update procedure and the current
pinned versions.
...
WinAppSDK 2.2 runtime (`Microsoft.WindowsAppRuntime.2`), so the XAML/WinUI layer is
