...
| Package output | In progress | The package backend runs, but 254 generated files still differ. `windows-sys` source output matches exactly. Rich Win32 has 167 differing files and WinRT has 85; source output has 1,823 missing and 2,004 extra line edges. The two manifests have 119 missing and 353 extra dependency edges. |
...
Intermediate parity batches use stricter regression limits than the final zero-difference gate. The
number of differing files and each manifest-edge direction may not increase. The sum of missing and
extra source-line edges must decrease, and neither source-line direction may increase by more than
2 percent. This permits a small temporary directional shift when restoring an omitted declaration
family produces a much larger net parity gain, without allowing missing output to be exchanged for
unbounded extra output.

WinRT package methods and ABI slots derive their namespace cfg gates from the lowered method
...
Native package item cfgs use the same transitive package dependency product as package feature
selection. A cached interface-method dependency product adds inherited base-method requirements to
implementation and runtime artifacts without changing manifest policy. Native trait inference
matches emitted packed, aligned, and union types, which remain `Copy` when nested. Rich COM wrappers
use owner-aware public types and retain ABI-to-owned transmutation for non-copy retval values.
Callable signatures retain `DoesNotReturnAttribute` and render `!` in raw links and rich wrappers.
Package sys capability checks reject direct noncanonical WinRT value types while retaining WinRT
interfaces that erase to raw ABI pointers.
...
Interface-valued native dependencies retain transitive base-interface identities after their ABI
...
Union ownership uses semantic copyability independently from derive support, so packed copyable
layouts remain `Copy` while non-copy union fields use `ManuallyDrop`.
Structs containing explicit-layout fields use manual bitwise `Clone` without broadening parent
ownership traits.
