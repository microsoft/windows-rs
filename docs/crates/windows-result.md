...
for contributors and is **not needed to use `windows-result`**.
...
[#3783](https://github.com/microsoft/windows-rs/pull/3783) ("Promote remaining
...
- **`WIN32_ERROR` is entirely synthetic.** It appears zero times in the source
  metadata. Win32 error constants are typed `u32` (`const ERROR_ACCESS_DENIED:
...
- **The repo already leaves a long tail of equally-legitimate status types
  generated per-namespace:** `JET_ERR` (232 returns), `CONFIGRET` (224),
...
Before acting, also confirm what de-promotion would require in the generated
`windows` crate (how those namespaces would reference the types once no longer
sourced from `windows-result`).
