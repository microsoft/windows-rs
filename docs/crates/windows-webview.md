...
This crate takes the opposite approach to `webview2-com`. That crate re-exports
the *entire* raw WebView2 COM API (every interface generated by `windows`) plus
proc-macro-generated callback adapters, and leaves callers to drive the COM
objects directly — maximal surface, larger build, raw pointers and `unsafe` at
the call site. `windows-webview` instead exposes a curated, safe, idiomatic API
over a *minimal* hand-picked slice of that COM surface, hides the COM types
behind owned Rust values, and avoids proc macros via `implement_decl`. The
parity table below tracks every capability the production ecosystem relies on so
the smaller surface stays a complete one.
...
environment options, and the navigation/title/window/permission/new-window
events.
...
These are the WebView2 capabilities the top dependents (`wry` → `tauri`) call
that this crate needed to be a viable backend for that ecosystem — now all
exposed. The remaining out-of-scope items below are feature-gated or worked
around by those crates.
...
| Profile (`ICoreWebView2Profile`/`Profile2`) | tauri | ✅ Done | `WebView::profile` returns a `Profile` (name, path, private mode, download folder, `preferred_color_scheme`, `clear_browsing_data_all`). |
| `NavigateWithWebResourceRequest` (`ICoreWebView2_2`) | wry | ✅ Done | `WebView::navigate_with_request` + `NavigationRequest` builder (method, headers, body). |
...
| `SetAllowExternalDrop` (`Controller4`) | wry | ✅ Done | `Controller::set_allow_external_drop`. |
| `ContainsFullScreenElementChanged` event | tauri-runtime-wry | ✅ Done | `WebView::on_contains_fullscreen_element_changed` + `contains_fullscreen_element`. |
| Environment options breadth (`AreBrowserExtensionsEnabled`, scrollbar style) | wry | ✅ Done | `EnvironmentOptions::are_browser_extensions_enabled` and `scrollbar_style` (`ScrollBarStyle`). |
| `SetMemoryUsageTargetLevel` (`ICoreWebView2_19`) | wry | ✅ Done | `WebView::set_memory_usage_target_level` (`MemoryUsageTargetLevel`). |
| Iframe/worker resource filters (`AddWebResourceRequestedFilterWithRequestSourceKinds`, `_22`) | wry | ✅ Done | `on_web_resource_requested` transparently covers every request source on supported runtimes, falling back to document-only otherwise. |

Out of scope for parity — wry feature-gates these or works around them, and
`webview2-com` only reaches them because it re-exports the entire raw COM API:
`AddHostObjectToScript` (wry uses `postMessage` instead),
`CallDevToolsProtocolMethod`, `TrySuspend`/`Resume`, `CapturePreview`,
