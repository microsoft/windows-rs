...
closure on a pool thread, `try_submit` returns the closure if submission fails, `for_each` runs a
closure over an iterator in parallel and waits for completion, and the `Pool` type offers direct
...
`Pool` and the `submit`/`try_submit`/`for_each` helpers are hand-written.
