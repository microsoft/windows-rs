...
Integer-valued pointer constants remain supported. If an interface-pointer typedef is projected as
the interface itself, constants declared with that typedef are omitted because ECMA metadata cannot
encode an interface-valued constant. An explicit pointer to the same interface remains a pointer and
is emitted.
