...
| `FARPROC` should always return `isize`. | The SDK declares `int` on x86 and `INT_PTR` on 64-bit. `INT_PTR` already collapses through the pointer-sized ABI rule, but the x86 branch contains only raw `int`. The same pattern exists only for `FARPROC`, `NEARPROC`, and `PROC`. | Keep the architecture-specific declarations in the literal scrape. A generic cross-architecture merger could recognize `i32` on x86 versus `isize` on 64-bit, but that would be an explicit normalization rule. |
